...
const registerPost = async(req, res) => {
  ...
  if(error) {
    ...
    return res.status(400).render('create-user', {errMessage:errMessage, createdUser: null });
  } else {
    const user = new User({
      ...req.body
    });
    await user.save();
    return res.status(400).render('create-user', {errMessage:null, createdUser: user });
  }
